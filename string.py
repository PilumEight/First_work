...
def donuts(count):
    if int(count) >= 10:
        ...
        return print('Количество пончиков: {0}'.format(count))
...
def both_ends(s):
    ...
    if len(s) < 2:
        ...
        return s[0:2:1] + d[len(d) - 2:len(d):1]
...
def front_back(a, b):
    alength = len(a)
    blength = len(b)
    if (alength % 2 == 0) and (blength % 2 == 0):
        result = a[0:int(alength / 2)] + b[0:int(blength / 2)] + a[int(alength / 2):] + b[int(blength / 2):]
    elif (alength % 2 == 1) and (blength % 2 == 1):
        result = a[0:int(alength / 2) + 1] + b[0:int(blength / 2) + 1] + a[int(alength / 2) + 1:] + b[int(
            blength / 2) + 1:]
    elif (alength % 2 == 1) and (blength % 2 == 0):
        result = a[0:int(alength / 2) + 1] + b[0:int(blength / 2)] + a[int(alength / 2) + 1:] + b[int(blength / 2):]
    elif (alength % 2 == 0) and (blength % 2 == 1):
        result = a[0:int(alength / 2)] + b[0:int(blength / 2) + 1] + a[int(alength / 2):] + b[int(blength / 2) + 1:]
    return result
